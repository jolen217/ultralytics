...
class TestNonMaxSuppression:
    ...
    def test_end2end_class_filter(self):
        pred = torch.zeros(1, 10, 6)
        pred[0, 0] = torch.tensor([10., 10., 50., 50., 0.9, 0.])
        pred[0, 1] = torch.tensor([20., 20., 60., 60., 0.8, 1.])
    ...
    def test_labels_appended(self):
        pred = torch.zeros(1, 84, 6300)
        labels = [torch.tensor([[0., 100., 100., 50., 50.]])]  # cls=0, xywh box
    ...
    def test_time_limit_warning(self):
        ...
        pred[0, :4, 0] = torch.tensor([100., 100., 50., 50.])
        pred[0, 4, 0] = 0.9
        pred[1, :4, 0] = torch.tensor([200., 200., 50., 50.])
        pred[1, 4, 0] = 0.85
